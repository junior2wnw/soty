...
async function runScenarios({ relayUrl } = {}) {
  ...
  assert(main.includes("agent.system === true"));
      ...
      assert(agent.includes('managedUserName || "") !== "Соты"') || agent.includes('managedUserName || "") !== "РЎРѕС‚С‹"'));
      ...
      assert(!agent.includes("I am not dropping the task"));
    ...
    ["agent installers stay lightweight by default", async () => {
      ...
      assert(ui.includes("openTaskMenu"));
      assert(ui.includes("exportSotyStateFile"));
      ...
      assert(reinstallPrepare.includes("Soty app state auto-backup/auto-import is disabled for reinstall"));
      assert(reinstallPrepare.includes('sotyAppStateMode = "manual-export-import"'));
      assert(!reinstallPrepare.includes("operator/export"));
      assert(!reinstallPrepare.includes("operator/import"));
      assert(!reinstallPrepare.includes("Edge-IndexedDB"));
      ...
      assert(ui.includes("agentDialogReplyTimeoutMs = 24 * 60 * 60_000"));
      assert(ui.includes("agentThinking.has(tunnelId) && !message"));
      assert(!ui.includes("if (agentThinking.has(tunnelId)) {\n    stopAgentDialogReply(tunnelId);"));
      assert(ui.includes("return Math.max(1000, Math.min(Math.trunc(timeoutMs), agentDialogReplyTimeoutMs));"));
      assert(!ui.includes("operatorTargetBusy"));
      assert(!ui.includes("\"! busy\""));
      ...
      assert(agentFeature.includes("agentRelayReplyTimeoutMs = 24 * 60 * 60_000"));
      assert(agentFeature.includes("pendingRelayReplyTtlMs = agentRelayReplyTimeoutMs + 30 * 60_000"));
