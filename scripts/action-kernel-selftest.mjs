  ...
  const cases = [
    ...
    ["real relay routes user jobs to user companion and system jobs to machine worker", async () => {
      ...
      try {
        ...
        assert(String(blockedUser.body.text || "").includes("user desktop session is not available"));
    ...
    ["cli toolkit describe works", async () => {
      ...
      assert(cli.stdout.includes("computer"));
      assert(cli.stdout.includes("soty_computer"));
    }],
    ...
    ["windows reinstall scripts default to managed Cyrillic passwordless account", async () => {
      ...
      const main = await readFile(join(root, "src", "main.ts"), "utf8");
      const webController = await readFile(join(root, "src", "features", "web-controller.ts"), "utf8");
      ...
      assert(main.includes("installWebController({"));
      assert(webController.includes("SOTY.remote"));
      assert(webController.includes("SOTY_REMOTE_CONTROLLER"));
      assert(webController.includes("soty.web-controller.v1"));
      assert(webController.includes("resolveWebControllerTarget"));
      assert(main.includes("webControllerTargets"));
      assert(main.includes("webControllerPending"));
      ...
      assert(agent.includes("$p.PSObject.Properties[($prefix + '.cx')]"));
      assert(agent.includes("$p.PSObject.Properties[($prefix + '.cy')]"));
      assert(agent.includes("clean-codex+memory-plane+computer-use-plane"));
      assert(agent.includes("computer"));
      ...
      assert(agent.includes("return tools.filter((tool) => sotyMcpPublicTools.includes(tool.name))"));
      assert(agent.includes("The user's source device does not need image credentials"));
      assert(agent.includes("Image generation is a native OpenAI built-in"));
      assert(!agent.includes('if (name === "soty_image")'));
      ...
      assert(agent.includes("interactive-launcher.vbs"));
      assert(agent.includes("shell.Run(command, 0, True)"));
      assert(agent.includes("New-ScheduledTaskAction -Execute $wscript"));
      ...
      assert(!agent.includes("image-generation-unavailable"));
      assert(!agent.includes("generateOpenAiImageData"));
      assert(!agent.includes("callSotyImageTool"));
      assert(!agent.includes("openAiFetchJsonViaProxy"));
      assert(!agent.includes("connectViaSocksProxy"));
      ...
      assert(agent.includes("safeOperatorTextLength"));
      assert(agent.includes("maxTextLength: sourceMaxTextLength"));
      ...
      assert(agent.includes("Computer-use plane:"));
      assert(agent.includes("use `computer` as one computer-use plane"));
      assert(agent.includes("Legacy `soty_*` names are hidden compatibility aliases"));
      ...
      assert(agent.includes("windowsImageSha256"));
      assert(agent.includes("media.spec"));
      ...
      assert(agent.includes("reinstallPrepareOrphanGraceSeconds"));
      ...
      assert(main.includes("explicitMention"));
      assert(main.includes("selectedAgentMode"));
      assert(main.includes("appendAgentPrivateLine"));
      assert(!main.includes("privateMode: true"));
      ...
      assert(managed.includes("Test-MediaDownloadProcess"));
      assert(!managed.includes('CommandLine -match "WindowsReinstall|Windows11_|\\.download|dl\\.delivery|Soty Windows reinstall image"'));
      ...
      assert(managed.includes("activePrepareProcessCount"));
      assert(managed.includes('[ValidateSet("preflight", "prepare", "status", "repair", "arm", "cancel")]'));
      assert(managed.includes("Invoke-ManagedRepair"));
      assert(managed.includes("nextAction"));
      assert(managed.includes("$script:PrepareOrphanGraceSeconds = 120"));
      ...
      assert(managed.includes("Get-MinimumUsbFreeGB"));
      assert(managed.includes("WindowsMediaManifestUrl"));
      assert(managed.includes("windows-media-spec.json"));
      assert(managed.includes("minimumUsbFreeGB"));
      ...
      assert(prepare.includes("Resolve-WindowsInstallMediaSpec"));
      assert(prepare.includes("Write-WindowsMediaSpecReceipt"));
      assert(prepare.includes("windows-media-spec.json"));
      assert(prepare.includes("Get-MinimumUsbFreeGB"));
      assert(prepare.includes("Unsupported Windows media format"));
      ...
      assert(prepare.includes("Preferred Windows edition policy"));
      assert(prepare.includes("Clear-ReinstallArmMarkers"));
      assert(prepare.includes("Test-WindowsEditionMatch"));
      assert(prepare.includes("function Copy-LargeFileToUsb"));
      assert(prepare.includes("/J"));
      assert(prepare.includes("Source profile is the managed reinstall user"));
    ...
    ["public manifest still validates after fallback build", async () => {
      ...
      assertEqual(manifest.memoryPlane.routeProfileSchema, "soty.route-profiles.v1");
      assertEqual(manifest.opsSkill, undefined);
      assertEqual(manifest.routeProfiles.schema, "soty.route-profiles.v1");
      ...
      assert(manifest.routeProfiles.profiles[0].route.some((item) => item.includes("windows-media manifest")));
      assert(manifest.routeProfiles.profiles[0].route.some((item) => item.includes("valid existing USB install image")));
      assert(manifest.routeProfiles.profiles[0].doNot.some((item) => item.includes("verified hosted media manifest")));
      assert(manifest.routeProfiles.profiles[0].proof.includes("mediaSpec"));
      ...
      assertEqual(manifest.computerUsePlane.legacyEntrypoint, "soty_computer");
      assert(manifest.computerUsePlane.standardTools.includes("computer"));
      ...
      assertEqual(reinstallToolkit.entryTool, "computer");
      assertEqual(reinstallToolkit.routeProfile, "soty-windows-reinstall-managed-fast-lane");
    ...
    ["agent installers stay lightweight by default", async () => {
      ...
      const miniAppRunnerHtml = await readFile(join(root, "public", "mini-app-runner.html"), "utf8");
      ...
      const sotyFieldMap = await readFile(join(root, "src", "ui", "soty-field-map.ts"), "utf8");
      const agentFeature = await readFile(join(root, "src", "features", "agent.ts"), "utf8");
      const syncSource = await readFile(join(root, "src", "sync.ts"), "utf8");
      const agentSource = await readFile(join(root, "scripts", "soty-agent.mjs"), "utf8");
      ...
      assert(windowsMachineInstall.includes("$installArgs += @('-Revision',$env:SOTY_AGENT_INSTALLER_REVISION)"));
      assert(!windowsMachineInstall.includes("'-Base',$env:SOTY_AGENT_INSTALL_BASE,'-Revision'"));
      ...
      assert(agentFeature.includes("$installArgs += @('-Revision',$env:SOTY_AGENT_INSTALLER_REVISION)"));
      assert(!agentFeature.includes("'-Base',$env:SOTY_AGENT_INSTALL_BASE,'-Revision'"));
      ...
      assert(ui.includes("Инструменты"));
      assert(ui.includes("+ инструменты"));
      ...
      assert(agentSource.includes("private agent mode defaults to the current/source computer when it exists"));
      ...
      assert(ui.includes("applySyncedWriterLines"));
      assert(ui.includes("linkifyChatLine"));
      assert(!ui.includes("current.time === time"));
      assert(ui.includes("bubble-mark"));
      assert(ui.includes('target="_blank" rel="noopener noreferrer"'));
      assert(ui.includes('class="bubble-avatar"'));
      assert(!ui.includes("<b>${escapeHtml(bubble.nick)}</b>"));
      assert(syncSource.includes("lineMeta"));
      assert(syncSource.includes("replaceWriterLineMeta"));
      assert(syncSource.includes("writerLineSnapshot"));
      assert(ui.includes("if (isAgentChromeLineClass(state.className))"));
      assert(ui.includes("const hasAgentThinking = showDialog && agentThinking.has(selectedId)"));
      ...
      assert(ui.includes("containsAgentInvocation"));
      assert(ui.includes("stripAgentInvocation"));
      assert(ui.includes("explicitMention"));
      assert(ui.includes("agent-private-panel"));
      assert(!ui.includes("privateMode: true"));
      ...
      assert(ui.includes("ensurePermanentCells"));
      assert(ui.includes("chooseCanonicalPermanentCell"));
      assert(ui.includes("forgetDuplicateCells"));
      ...
      assert(styles.includes(".bubble-time"));
      assert(styles.includes(".bubble-avatar"));
      assert(styles.includes(".chat-bubble.remote .bubble-avatar"));
      assert(styles.includes(".chat-bubble.local .bubble-avatar"));
      assert(styles.includes(".chat-bubble p a"));
      ...
      assert(ui.includes("renderChatSwitcher(sorted)"));
      ...
      assert(sotyFieldOverview.includes("renderSotyFieldMap"));
      assert(sotyFieldOverview.includes("resetSotyFieldMap"));
      assert(sotyFieldOverview.includes("soty-field-overview-cell"));
      assert(sotyFieldOverview.includes("soty-field-overview-search"));
      assert(sotyFieldOverview.includes("soty-field-overview-map"));
      assert(sotyFieldOverview.includes("data-soty-field-search"));
      assert(sotyFieldOverview.includes("applySotyFieldSearch"));
      assert(sotyFieldMap.includes("renderSotyFieldMap"));
      assert(sotyFieldMap.includes("soty-field-map-canvas"));
      assert(sotyFieldMap.includes("gridPositions"));
      assert(sotyFieldMap.includes("longPressMs"));
      assert(sotyFieldMap.includes("zoomAt"));
      assert(sotyFieldMap.includes("actions.menu"));
      ...
      assert(styles.includes(".soty-field-overview-map"));
      assert(styles.includes(".soty-field-map-cell"));
      assert(ui.includes("renderSotyAppDockSurface"));
      assert(ui.includes("soty-app-dock"));
      assert(sotyAppDock.includes("renderSotyAppDock"));
      assert(styles.includes(".soty-app-dock"));
      assert(ui.includes("publishInlineMiniAppRunner"));
      assert(ui.includes("miniAppRunnerUrl"));
      assert(ui.includes("soty.mini-app.runner.v1"));
      assert(ui.includes("allow-scripts allow-same-origin allow-forms allow-popups allow-downloads"));
      assert(!ui.includes("setMiniAppInlineFrameUrl"));
      assert(!ui.includes("miniAppInlineFrameUrl"));
      assert(miniAppRunnerHtml.includes("/mini-app-runner.js"));
      assert(miniAppRunnerHtml.includes("sandbox=\"allow-scripts allow-forms allow-popups allow-downloads\""));
      assert(miniAppRunnerJs.includes("soty.mini-app.runner.v1"));
      assert(miniAppRunnerJs.includes("appFrame.srcdoc"));
      assert(miniAppRunnerJs.includes("window.parent.postMessage"));
      assert(httpApp.includes('const scriptSrc = miniAppRunner ? "script-src'));
      assert(httpApp.includes("mini-app-runner.html"));
      assert(httpApp.includes("miniAppAsset || miniAppRunner"));
      ...
      assert(!ui.includes("hex-field"));
      assert(!styles.includes(".hex-field"));
      assert(!styles.includes(".hive-panel"));
      assert(!styles.includes(".hive-toggle"));
      assert(!styles.includes(".empty-hive"));
      ...
      assert(ui.includes("function personalInstallGuide(): NonNullable<PersonalSpaceInstallResult[\"guide\"]>"));
      assert(ui.includes("function shouldPreferManualPersonalInstallGuide(): boolean"));
      assert(ui.includes("function isIosDevice(): boolean"));
      assert(ui.includes("function isAndroidDevice(): boolean"));
      assert(ui.includes('setHeadLink("icon", iconSrc, "image/svg+xml");'));
      assert(ui.includes('setHeadLink("apple-touch-icon", touchIconSrc, touchIconType);'));
      assert(personalSpaceSource.includes("export type PersonalSpaceInstallGuide"));
      assert(personalSpaceSource.includes("function showInstallGuideSheet(root: HTMLElement, guide: PersonalSpaceInstallGuide): void"));
      ...
      assert(ui.includes("autoDownloadedFilesKey"));
      assert(syncSource.includes("autoDownload?: boolean"));
      assert(syncSource.includes("commandId?: string"));
      assert(syncSource.includes("delivery?: string"));
