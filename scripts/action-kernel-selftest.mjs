...
async function runScenarios({ relayUrl } = {}) {
  const cases = [
    ["health reports new version", async () => {
      ...
      assertEqual(health.body.version, "0.4.100");
  ...
  assert(main.includes("agent.system === true"));
    ...
    ["public manifest still validates after fallback build", async () => {
      ...
      assertEqual(manifest.version, "0.4.100");
    ...
    ["agent installers stay lightweight by default", async () => {
      ...
      assert(windowsMachineInstall.includes("soty-agent-machine-bootstrap:0.4.100"));
      ...
      assert(agentSource.includes('const agentVersion = "0.4.100"'));
    ...
    ["managed agent auto-updates and exits for runner restart", async () => {
      ...
      const oldSource = nextSource.replace('const agentVersion = "0.4.100";', 'const agentVersion = "0.4.65";');
