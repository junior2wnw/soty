...
async function writeCodexRuntimeFiles(jobDir, runtimeContext) {
  ...
  const localApiHelper = [
    ...
    "    const action = requestedAction && requestedAction !== operation ? requestedAction : 'open';",
    "    await scriptNode(browserNode({ ...req, action }), { name: 'computer-browser', timeoutMs: Math.max(1000, Math.min(Number(req.timeoutMs) || 60000, 120000)) });",
    "    return;",
    "  }",
    "  if (operation === 'browser') {",
