...
function buildAgentPrompt(text, context = "", runtimeContext = null) {
  ...
  const lines = [
    ...
    "- After native image generation, find the real output under the Codex home generated_images directory if needed, then move bytes with `computer` operation=artifact; never upload generated images to public temporary hosts or serve them with local HTTP.",
    ...
    const tools = [
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            downloadName: { type: "string", description: "Optional display filename for file action=download/publish." },
      ...
      {
        name: "soty_file",
        description: "Seamless Desktop-Commander-style file access on the current Soty Agent LINK source device. Use for listing, reading, writing, searching, moving, copying, deleting, creating project files, and publishing/downloading an exact source-device file through the encrypted Soty room file rail. Never use public upload services, temporary HTTP servers, or paste/base64 chat as a file-transfer fallback while this capability is available.",
        inputSchema: {
          type: "object",
          properties: {
            action: { type: "string", description: "One of: stat, list, read, write, append, mkdir, search, move, copy, delete, download, publish. download/publish sends the exact file through the Soty tunnel to the controlling room's file rail." },
            ...
            downloadName: { type: "string", description: "Optional display filename for action=download/publish." },
...
function publishFile(fullPath) {
  ...
  let index = 0;
  try {
    emitSotyFileControl("BEGIN", { id: fileId, name, type, size: stat.size, total });
    for (;;) {
  ...
  return { name, type, bytes: stat.size, chunks: total, sha256 };
}
...
try {
  ...
  if (action === "stat") {
    ...
    emit({ ok: true, action, path: fullPath, sentTo: "soty-room-file-rail", ...published });
