...
function sanitizeMiniAppInstallPayload(payload) {
  ...
  const height = safeMiniAppCssSize(rawDisplay.height || payloadDisplay.height || rawApp?.height || payload?.height || "");
  const width = safeMiniAppCssSize(rawDisplay.width || payloadDisplay.width || rawApp?.width || payload?.width || "");
...
function normalizeMiniAppLayout(value) {
...
function safeMiniAppCssSize(value) {
...
async function maybeWaitForWindowsReinstallTerminalAfterCodex({ taskFamily, source, target, finalText = "", trace = null, signal = null, shouldStop = null } = {}) {
...
function sotyRuntimeHints() {
  return [
    ...
    "- Linked-device canonical: in a selected device cell's private agent mode, or when the current request names Link devices, the selected/named Link targets are first-class computer-use planes through the controller.",
    ...
    "- Klava targeting: private agent mode defaults to the current/source computer when it exists. Use a Link device only when the user names it now, or when the request came from that selected device cell.",
...
async function writeCodexRuntimeFiles(jobDir, runtimeContext) {
  ...
  const routes = [
    ...
    "",
    "Bridge rule: apps request small capability-gated actions (`agent.invoke`, `terminal.run`, file/artifact/job capabilities as they are added); the shell always provides `window.collapse` and `window.resize` for panel UX. There is no close button/capability: collapse keeps the app available in the chat dock. The kernel executes through the selected chat/device context and returns proof/status.",
    "",
    ...
    "",
    "APPKA install/open rule: for generated inline helpers, call `computer` with `{ \"operation\": \"appka\", \"appId\": \"...\", \"title\": \"...\", \"tags\": [\"...\"], \"profileTitle\": \"...\", \"inlineHtml\": \"<!doctype html>...\", \"scope\": \"chat\", \"layout\": \"half\", \"capabilities\": [\"chat.append\"] }`. The inline app is synced in the selected Soty room, appears on the cell app shelf, and opens in the lower half of the cell immediately unless `open=false`; the app may later request `window.resize` with layout `compact`, `large`, `full`, or `floating` plus safe CSS height/width.",
    "",
    "URL install/open rule: after the frontend is available at a safe URL, call `computer` with `{ \"operation\": \"mini_app\", \"appId\": \"...\", \"title\": \"...\", \"tags\": [\"...\"], \"profileTitle\": \"...\", \"url\": \"...\", \"scope\": \"chat\", \"layout\": \"half\", \"capabilities\": [\"chat.append\"] }`. Starting a server or creating files is not enough; registration is what binds the surface into Soty. Use `scope=account` only for local account-wide tools and `scope=device` only for a proven selected device. The app will appear on the selected cell app shelf and in the all-app gallery unless `open=false`.",
    "",
...
function runMcpServer() {
    ...
    const tools = [
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            windowsMediaManifestUrl: { type: "string", description: "Optional hosted Windows media manifest URL. The manifest can provide downloadUrl/url, sha256, fileName, and sizeBytes for the reinstall image." },
            mediaManifestUrl: { type: "string", description: "Alias for windowsMediaManifestUrl." },
            windowsImageUrl: { type: "string", description: "Optional direct Windows install image URL fallback when no media manifest is used." },
            windowsImageSha256: { type: "string", description: "SHA-256 for windowsImageUrl." },
      ...
      {
        name: "soty_mini_app",
        description: "Register and optionally open a generated mini app on the selected Soty cell app shelf: account/chat/device scope, optional profile, tags, and visibility. For user requests like `сделай аппку`, prefer a self-contained inlineHtml bundle with scope=chat through the public `computer` tool operation=appka/mini_app.",
        inputSchema: {
          type: "object",
          properties: {
  ...
  async function callSotyReinstallTool(args) {
    ...
    const request = {
      ...
      manifestUrl: updateManifestUrl,
      panelSiteUrl: originFromUrl(updateManifestUrl) || agentRelayBaseUrl || "https://xn--n1afe0b.online",
      windowsMediaManifestUrl: String(args.windowsMediaManifestUrl || args.mediaManifestUrl || "").trim().slice(0, 4000),
      windowsImageUrl: String(args.windowsImageUrl || "").trim().slice(0, 4000),
      windowsImageSha256: String(args.windowsImageSha256 || "").trim().slice(0, 128),
  ...
  function compactReinstallStatus(status) {
    ...
    return {
      ...
      media: media ? {
        ...
        spec: media.spec && typeof media.spec === "object" ? {
          source: cleanActionText(media.spec.source || "", 40),
          manifestUrl: cleanActionText(media.spec.manifestUrl || "", 400),
          url: cleanActionText(media.spec.url || "", 400),
          fileName: cleanActionText(media.spec.fileName || "", 160),
          sha256: cleanActionText(media.spec.sha256 || "", 80),
          sizeBytes: Number.isFinite(Number(media.spec.sizeBytes)) ? Number(media.spec.sizeBytes) : 0
        } : null,
...
function sourceManagedWindowsReinstallBootstrap(args) {
  const payload = Buffer.from(JSON.stringify({
    ...
    windowsMediaManifestUrl: String(args.windowsMediaManifestUrl || args.mediaManifestUrl || "").slice(0, 4000),
    windowsImageUrl: String(args.windowsImageUrl || "").slice(0, 4000),
    windowsImageSha256: String(args.windowsImageSha256 || "").slice(0, 128),
...
try {
  ...
  if (-not [string]::IsNullOrWhiteSpace([string]$req.windowsMediaManifestUrl)) { $psArgs += @('-WindowsMediaManifestUrl', [string]$req.windowsMediaManifestUrl) }
  if (-not [string]::IsNullOrWhiteSpace([string]$req.windowsImageUrl)) { $psArgs += @('-WindowsImageUrl', [string]$req.windowsImageUrl) }
  if (-not [string]::IsNullOrWhiteSpace([string]$req.windowsImageSha256)) { $psArgs += @('-WindowsImageSha256', [string]$req.windowsImageSha256) }
