...
$usbMediaSources = Join-Path $usbRoot "sources"
  ...
  foreach ($root in $SourceRoots) {
    ...
    foreach ($name in @("install.swm", "install.esd", "install.wim")) {
      ...
      if (Test-Path -LiteralPath $path) {
        return [pscustomobject]@{ Path = $path; SourceRoot = $root }
      }
    }
  }
  return $null
...
$configText = Get-Content -LiteralPath (Join-Path $usbReinstall "config.cmd") -Raw
$installSourceRoots = if ($configText -match '(?im)^\s*set\s+INSTALL_SOURCE_ROOT=REINSTALL\s*$') {
  @($usbSources, $usbMediaSources)
} else {
  @($usbMediaSources, $usbSources)
}
$installImage = Get-InstallImageCandidate $installSourceRoots
if (-not $installImage) {
  throw "Required install image missing under $usbMediaSources or $usbSources."
}

$backupProof = $ready.backupProof
if (-not $backupProof) {
  throw "Backup proof missing from ready.json. Run managed prepare again before arming reinstall."
}
...
foreach ($path in @(
  ...
  (Join-Path (Join-Path (Join-Path (Join-Path $installImage.SourceRoot '$OEM$') '$$') "Setup\Scripts") "SetupComplete.cmd")
)) {
  if (-not (Test-Path -LiteralPath $path)) { throw "Required backup/OOBE artifact missing: $path" }
}
