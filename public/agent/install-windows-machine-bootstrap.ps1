...
$elevatedLines = @(
  ...
  '$installArgs = @(''-NoLogo'', ''-NoProfile'', ''-ExecutionPolicy'', ''Bypass'', ''-File'', $script, ''-Base'', $base, ''-Scope'', ''Machine'', ''-LaunchAppAtLogon'')',
  'if (-not [string]::IsNullOrWhiteSpace($relay)) { $installArgs += @(''-RelayId'', $relay) }',
