...
async function writeCodexRuntimeFiles(jobDir, runtimeContext) {
  ...
  const routes = [
    ...
    "",
    "APPKA install/open rule: for generated inline helpers, call `computer` with `{ \"operation\": \"appka\", \"appId\": \"...\", \"title\": \"...\", \"tags\": [\"...\"], \"profileTitle\": \"...\", \"inlineHtml\": \"<!doctype html>...\", \"scope\": \"chat\", \"layout\": \"half\", \"capabilities\": [\"chat.append\"] }`. The inline app is synced in the selected Soty room, appears in the chat Apps panel/gallery, and opens in the lower half of the chat immediately unless `open=false`; the app may later request `window.resize` with layout `compact`, `large`, `full`, or `floating` plus safe CSS height/width.",
    "",
    "URL install/open rule: after the frontend is available at a safe URL, call `computer` with `{ \"operation\": \"mini_app\", \"appId\": \"...\", \"title\": \"...\", \"tags\": [\"...\"], \"profileTitle\": \"...\", \"url\": \"...\", \"scope\": \"chat\", \"layout\": \"half\", \"capabilities\": [\"chat.append\"] }`. Starting a server or creating files is not enough; registration is what binds the surface into Soty. Use `scope=account` only for local account-wide tools and `scope=device` only for a proven selected device. The app will appear in the selected chat Apps panel and in the all-app gallery unless `open=false`.",
    "",
...
function runMcpServer() {
  ...
  function sotyMcpToolList() {
    const tools = [
      ...
      {
        name: "soty_mini_app",
        description: "Register and optionally open a generated mini app in the selected Soty chat Apps panel: account/chat/device scope, optional profile, tags, and visibility. For user requests like `сделай аппку`, prefer a self-contained inlineHtml bundle with scope=chat through the public `computer` tool operation=appka/mini_app.",
        inputSchema: {
          type: "object",
          properties: {
