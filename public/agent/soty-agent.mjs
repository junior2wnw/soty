...
async function writeCodexRuntimeFiles(jobDir, runtimeContext) {
  ...
  const routes = [
    ...
    "",
    "URL install/open rule: after the frontend is available at a safe URL, call `computer` with `{ \"operation\": \"mini_app\", \"appId\": \"...\", \"title\": \"...\", \"tags\": [\"...\"], \"profileTitle\": \"...\", \"url\": \"...\", \"scope\": \"chat\", \"layout\": \"half\", \"capabilities\": [\"chat.append\"] }`. Use `scope=account` only for local account-wide tools and `scope=device` only for a proven selected device. The app will appear on the selected cell app shelf and in the all-app gallery unless `open=false`.",
    "",
