...
test('exact reviewed queued work survives every admission and rollback checkpoint',async()=>{
 for(const rollback of [false,true]){
  const f=fixture(rollback?{op:'readiness',when:'before'}:{}),digest='1'.repeat(64),helper=f.run.maintenance;
  f.run.maintenance=async(verb,run)=>({...await helper(verb,run),count:2,activeJobs:[{id:'pending-one',status:'queued'},{id:'pending-two',status:'queued'}],queuedSha256:digest});
  await f.run.prepare({...args,preserveQueuedSha256:digest,legacyRecovery:true});
  assert.equal(f.run.state.legacyRecovery,true);
  if(rollback){await assert.rejects(f.run.promote());assert.equal(f.run.state.phase,'restored');}
  else{await f.run.promote();assert.equal(f.run.state.phase,'committed');}
  assert.equal(f.map.get('sentinel').State.Running,true);
 }
});
test('queued fingerprint drift after original stop prevents migration',async()=>{
 const f=fixture(),digest='1'.repeat(64),helper=f.run.maintenance;let statuses=0;
 f.run.maintenance=async(verb,run)=>({...await helper(verb,run),count:1,activeJobs:[{id:'pending',status:'queued'}],queuedSha256:verb==='status'&&++statuses>1?'2'.repeat(64):digest});
 await f.run.prepare({...args,preserveQueuedSha256:digest});
 await assert.rejects(f.run.promote(),/offline_admission_race/);
 assert.equal(f.run.state.phase,'restored');assert.ok(!f.events.includes('helper:enter'));
});
