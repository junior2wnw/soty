...
This backend update starts from production c0ca5b810d87e3da73f4ddba6dc98b1e8ac387a0 and retains the subsequently deployed concurrency limit of 100. It changes connector persistence and delivery. The backend Dockerfile derives from the exact current image and preserves its client assets, dependencies, installed1.2.12 runtime, Roy/OpenCode bindings, upstream model and credential format. It does not enable an optional application model policy or update the browser request UI.
...
`status` is read-only and reports active IDs/status/count and a pending-job fingerprint. By default `enter` and `rollback` require no active jobs. An explicit `--preserve-queued-sha256` may preserve an exact reviewed set of queued jobs that have never been leased: zero attempts, no assigned connector and no lease/start/retry events. The fingerprint covers their complete normalized bodies, metadata and event history. Both commands still require exclusive ownership and the exact serving container stopped. The fingerprint must match before stop, offline, after import and before admission; rollback must preserve it too. Assigned, previously leased, added, changed or missing work fails closed. The old c0ca process ignores the maintenance marker; a supervisor must separately establish absence of pending old writes. Counts and fingerprints alone do not prove an empty in-memory write queue.
