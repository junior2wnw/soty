...
app-surface and installed-agent runtime contracts, and other generic reliability primitives
...
- `docs/soty-agent-runtime.md` documents the Soty adapter for the reusable `trustlink-kernel/docs/agent-runtime.md` contract.
...
Long agent tasks are expected to survive ordinary install/download/repair waits. The browser and server-side Codex turn wait through long jobs, the server relay keeps source jobs for the same all-day window, and Soty MCP tools accept `timeoutMs` up to `86400000` for long scripts instead of cutting work at the old five-to-ten-minute ceiling. When a job is still running, the agent should keep ownership by polling durable status with `waitMs`; users should only be asked for destructive confirmation, credentials, physical action, or a proven device outage.
...
Agent chat uses the server stock Codex CLI with native OpenAI/Codex tools enabled where the CLI supports them, plus one Soty MCP server. OpenAI built-ins such as web search and image generation remain native tools; Soty MCP exposes the selected user's computer as `computer` and does not reimplement or shadow built-in tool names. For a generated wallpaper, Codex generates with native image generation, transfers the exact `$CODEX_HOME/generated_images/...` file through `computer` `operation=artifact`, then applies it with `computer` `operation=wallpaper` and verifies source-device proof. For a file that already lives on a granted device, Codex uses `computer` file `action=download` when the controller/current computer should receive it in browser Downloads, and `action=publish` only when the user wants the encrypted room file rail. Public upload services, temporary local HTTP servers, pasted base64, and "download this from a third-party link" are not the normal file-transfer route. To route the server Codex executor through a network proxy, set `SOTY_CODEX_PROXY_URL` on that server executor. Local device agents do not run Codex as the chat brain. `/health` reports only `codexProxy: true` and the proxy scheme.
